...
export const ja = {
	...
	settings: {
		...
		model: {
			...
			desc:
				...
				"エイリアスは CLI が最新バージョンへ解決します（default = アカウント既定、best = Fable 5 が使えればそれ・なければ最新 Opus、opusplan = プラン作成のみ Opus・実行は Sonnet）。" +
				"[1m] 付きは同じモデルの 1M（100万）トークン・コンテキスト版で、通常版（200K）より履歴の自動要約（auto-compaction）までの余裕が約5倍になる反面、応答が重くなりやすくレート制限も速く消費します。" +
				"プランによっては 1M 版を使えないことがあり、その場合はエラーになるので通常版に戻してください。",
		},
		thinking: {
			name: "思考（拡張思考）",
			desc:
		...
		includeActiveDefault: {
			name: "アクティブファイル／フォルダを初期状態で含める",
			desc:
				"オンにすると、チャットパネルを開いた時に、" +
				"アクティブファイル（またはフォルダ）をプロンプトに含めた状態で始まります。" +
				"オフのときは除外された状態で始まります。" +
				"いずれもパネル内の「含める／除外」トグルで都度切り替えられます。",
		},
		saveAttachments: {
	...
	slash: {
		desc: {
			...
			think: "思考深度の表示 / 変更",
			mcp: "MCP サーバの状態を表示",
			plugin: "プラグインの一覧 / インストール / 削除（claude plugin に転送）",
		...
		help: {
			...
			itemPlugin: "プラグイン操作（list / install / uninstall / enable / disable / update / details）",
		...
		plugin: {
			title: (cmd: string) => `プラグイン: \`claude plugin ${cmd}\``,
			running: (cmd: string) => `\`claude plugin ${cmd}\` 実行中…`,
			error: (msg: string) => `エラー: ${msg}`,
			emptyOutput: "(出力はありません)",
			exitCode: (code: number) => `(終了コード ${code})`,
			usage: [
				"**使い方:**",
				"- `/plugin` または `/plugin list` — インストール済み一覧",
				"- `/plugin install <name>@<marketplace>` — インストール",
				"- `/plugin uninstall <name>@<marketplace>` — アンインストール",
				"- `/plugin enable <name>@<marketplace>` — 有効化",
				"- `/plugin disable [<name>@<marketplace>]` — 無効化",
				"- `/plugin update <name>@<marketplace>` — 更新",
				"- `/plugin details <name>` — 詳細表示",
				"",
				"その他のサブコマンド（`marketplace`, `validate`, `tag`, `prune` など）も同じ形式で `claude plugin ...` に転送されます。",
			].join("\n"),
		},
		mcp: {
			title: "MCP サーバ（ライブ）",
